...
export default function ValuationClient() {
  ...
  const quoteQuery=useApiQuery<PriceQuote>('/api/price',undefined,{refreshInterval:30_000});
  const snapshotQuery=useApiQuery<ValuationSnapshot>('/api/valuation/snapshot',undefined,{refreshInterval:600_000});
  const histQuery=useApiQuery<{points:ValuationPoint[]}>('/api/valuation/history',{period:'all'});
  const ageQuery=useApiQuery<{period:string;points:AgePoint[]}>('/api/valuation/hodl-waves',{period});
  const dormQuery=useApiQuery<{period:string;points:DormancyPoint[]}>('/api/valuation/dormancy',{period});
  const poolsQuery=useApiQuery<Pools>('/api/pools/overview');
  const searchQuery=useApiQuery<{snapshot:{capturedAt:string}|null}>('/api/valuation/search-interest',undefined,{refreshInterval:300_000});
