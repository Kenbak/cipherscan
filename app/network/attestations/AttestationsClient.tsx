  ...
  return <article id={endpoint.id} className="card p-0! overflow-hidden scroll-mt-28">
    <div className="p-5 sm:p-6">
      ...
      <div className="mt-5 text-xs text-muted flex flex-wrap justify-between gap-x-4 gap-y-2">
        ...
        {hub ? <a href={`#${hub.id}`} className="hover:text-cipher-cyan">Configured hub: {hub.name} · {STATES[attestationStatus(hub.latest, now)]?.label.toLowerCase()} ↓</a> : <span>{endpoint.role === 'shim' ? 'Hub configuration not confirmed' : 'Hub observed independently'}</span>}
      </div>
    </div>
