...
export const metadata = buildPageMetadata({
  title: 'Zero Indexer Attestation Monitor | CipherScan',
  ...
  imageAlt: 'CipherScan Zero Indexer attestation monitor',
});
...
export default async function AttestationsPage() {
  ...
  try {
    const response = await fetchWithDeadline(`${getApiUrl()}/api/network/attestations`, { cache: 'no-store' });
    if (response.ok) {
      const data = await response.json();
      if (data.success === true && data.network === network && Array.isArray(data.endpoints)) initialData = data;
    }
  } catch { /* The introduction and explicit unavailable state still render on an API outage. */ }
  const publicApiUrl = normalizeApiBaseUrl(process.env.NEXT_PUBLIC_API_URL || `https://api.${network}.cipherscan.app`);
  ...
  return <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12">
    ...
    <Link href="/network" className="inline-block text-sm text-muted hover:text-cipher-cyan mb-6">← Network overview</Link>
    <PageHeader eyebrow="NETWORK / ATTESTATIONS" title="Zero Indexer attestations"
    ...
    <section className="mt-12 border-t border-cipher-border pt-8" aria-labelledby="attestation-method">
      ...
      <p className="text-sm text-muted mt-6 max-w-4xl">Canary adds signed observations for endpoints with reproduced build measurements, checked every minute and expiring after three minutes. It runs on CipherScan’s server with operator-pinned signing keys; Canary’s own runtime is not enclave-attested. Connection warnings remain visible even when a recent signed check passed.</p>
      <p className="text-sm text-muted mt-6 max-w-4xl">These are periodic observations from CipherScan, not verification of your wallet’s connection. A passing shim does not establish its hub’s status. Build tags identify reviewed source commits. A matching reproduced configuration can confirm a shim’s configured hub, but does not prove live routing or hub availability. The monitor does not measure transaction privacy, batching or wallet service uptime.</p>
      <div className="flex flex-wrap gap-x-6 gap-y-3 mt-5 text-sm">
        <a className="text-cipher-cyan hover:underline" href="https://docs.caution.co/concepts/attestation/" target="_blank" rel="noopener noreferrer">Attestation methodology ↗</a>
        <a className="text-cipher-cyan hover:underline" href="https://docs.caution.co/guides/verify-an-app/" target="_blank" rel="noopener noreferrer">Verify a build yourself ↗</a>
        <a className="text-cipher-cyan hover:underline" href={`${publicApiUrl}/api/network/attestations`}>Public JSON API ↗</a>
      </div>
    </section>
  </div>;
}
