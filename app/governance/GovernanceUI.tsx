...
export function Status({ state }: { state: VoteState | 'announced' }) {
  const color = state === 'results' ? 'border-cipher-yellow/30 bg-cipher-yellow/5 text-cipher-yellow' : state === 'active' ? 'border-cipher-cyan/30 bg-cipher-cyan/5 text-cipher-cyan' : 'border-cipher-border text-secondary';
  return <span className={`inline-flex shrink-0 rounded-md border px-2 py-1 font-mono text-[10px] uppercase tracking-wider ${color}`}>{state === 'announced' ? 'Upcoming' : STATE_LABELS[state]}</span>;
}
...
export function GovernanceShell({ title, description, path, children, back = true }: { title: string; description: string; path: string; children: React.ReactNode; back?: boolean }) {
  ...
  return <div className="mx-auto max-w-7xl px-4 py-6 sm:px-6 sm:py-8 lg:px-8">
    ...
    <GovernanceRefresh />
    {back && <Link href="/governance" className="mb-4 inline-block font-mono text-xs text-muted hover:text-cipher-cyan">← All votes</Link>}
...
export function Freshness({ catalog }: { catalog: Catalog }) {
  ...
  return <p role={stale ? 'status' : undefined} className={`mt-5 text-xs ${stale ? 'text-cipher-yellow' : 'text-muted'}`}>
...
export function SourceNote() {
  return <div className="mt-8 border-t border-cipher-border pt-4 text-xs leading-relaxed text-muted">
    <p>CipherScan displays reviewed rounds and rounds endorsed by ZODL on the Valar voting chain. API publication does not mean CipherScan has independently verified a tally.</p>
    <p className="mt-1">Votes are cast through supported wallets. CipherScan never handles votes or keys.</p>
  </div>;
}
