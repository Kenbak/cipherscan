...
export async function generateMetadata({ params }: Props) {
  ...
  if (!validSlug(slug) || getNetwork() !== 'mainnet') return buildPageMetadata({ title: 'Vote Not Found | CipherScan', description: 'This vote could not be found.', path: '/governance', canonical: false, index: false });
  ...
  return buildPageMetadata({ title: `${title} — Voting & Results | CipherScan`, description: vote ? (vote.description.slice(0, 220) || `Explore ${vote.title}: ballot options, published results, and independent tally verification.`) : slug === GRANTS.slug ? GRANTS.description : 'Vote data is temporarily unavailable.', path: vote?.href ?? `/governance/${slug}`, index: Boolean(vote) || slug === GRANTS.slug, networks: ['mainnet'] });
}
...
export default async function VotePage({ params }: Props) {
    ...
    return <GovernanceShell title={GRANTS.title} description={GRANTS.description} path={`/governance/${GRANTS.slug}`}>
      <div className="rounded-xl border border-cipher-border bg-cipher-surface p-4 sm:p-5">
        ...
        <div className="mt-5 grid gap-4 sm:grid-cols-3">
          {[["Review closes", 'September 16 · 20:00 UTC'], ['Scheduled voting', 'September 17–29, 2026'], ['Proposals', '37 grant applications']].map(([label, value]) => <div key={label}><p className="font-mono text-[10px] uppercase tracking-wider text-muted">{label}</p><p className="mt-1 text-sm font-medium text-primary">{value}</p></div>)}
        </div>
        ...
        <div className="mt-4 flex flex-wrap gap-4 text-sm"><a href={GRANTS.source} target="_blank" rel="noopener noreferrer" className="text-cipher-cyan hover:underline">Review all proposals ↗</a><a href={GRANTS.calendar} target="_blank" rel="noopener noreferrer" className="text-secondary hover:underline">Governance calendar ↗</a></div>
      </div>
  ...
  return <GovernanceShell title={vote.title} description={vote.description} path={vote.href}>
    ...
    <section className="mt-6 rounded-xl border border-cipher-border p-4 sm:p-5"><h2 className="text-sm font-semibold text-primary">Sources & independent verification</h2>
      ...
      <div className="mt-4 flex flex-wrap gap-4 text-xs text-cipher-cyan"><a href={`${GOVERNANCE_API}/vote-summary/${vote.id}`} target="_blank" rel="noopener noreferrer">API summary ↗</a><a href={`${GOVERNANCE_API}/tally-results/${vote.id}`} target="_blank" rel="noopener noreferrer">Raw tally ↗</a><a href="https://tally.valargroup.org" target="_blank" rel="noopener noreferrer">Verification guide ↗</a></div>
