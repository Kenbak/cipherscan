...
function VoteCard({ vote }: { vote: Vote }) {
  return <Link href={vote.href} className="group block rounded-xl border border-cipher-border bg-cipher-surface p-4 transition-colors hover:border-cipher-cyan/40 sm:p-5">
    <div className="flex flex-wrap items-center justify-between gap-3"><h3 className="text-base font-semibold text-primary group-hover:text-cipher-cyan">{vote.title}</h3><Status state={vote.state} /></div>
    <p className="mt-2 line-clamp-2 max-w-3xl text-sm leading-relaxed text-secondary">{vote.description}</p>
    <div className="mt-4 flex flex-wrap items-center justify-between gap-2 font-mono text-xs text-muted"><span>{vote.round.proposals.length} questions · {vote.state === 'active' ? 'Closes' : 'Voting deadline'} {new Date(vote.round.vote_end_time * 1000).toLocaleDateString('en-GB', { timeZone: 'UTC', day: 'numeric', month: 'short', year: 'numeric' })}</span><span className={vote.state === 'results' ? 'text-cipher-yellow' : 'text-cipher-cyan'}>{vote.state === 'results' ? 'Explore results & verification' : 'View vote'} →</span></div>
  </Link>;
}
  ...
  return <GovernanceShell title="Zcash Governance" description={description} path="/governance" back={false}>
    ...
    <div className="space-y-7">
      ...
      {(announced || upcoming.length > 0) && <section aria-labelledby="upcoming-votes"><h2 id="upcoming-votes" className="mb-3 font-mono text-xs uppercase tracking-wider text-muted">{announcementDue && !upcoming.length ? 'Awaiting confirmation' : 'Upcoming'}</h2><div className="space-y-3">
        {announced && <Link href={`/governance/${GRANTS.slug}`} className="group block rounded-xl border border-cipher-cyan/25 bg-cipher-surface p-4 sm:p-5">
          <div className="flex flex-wrap items-center justify-between gap-3"><h3 className="text-base font-semibold text-primary group-hover:text-cipher-cyan">{GRANTS.title}</h3>{announcementDue ? <span className="font-mono text-xs text-muted">Schedule announced · awaiting round</span> : <Status state="announced" />}</div>
          <p className="mt-2 text-sm text-secondary">37 proposals for completed work across the Zcash ecosystem.</p>
          <div className="mt-4 flex flex-wrap justify-between gap-2 font-mono text-xs"><span className="text-muted">September 17–29, 2026 · Official schedule</span><span className="text-cipher-cyan">Review the proposals →</span></div>
        </Link>}
