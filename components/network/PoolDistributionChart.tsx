  ...
  return (
    ...
    >
      <ResponsiveContainer width="100%" height={320}>
        {hasBreakdown && chartData.some((d) => d.orchard > 0) ? (
          ...
          <AreaChart data={chartData}>
            <CartesianGrid strokeDasharray="2 6" stroke={colors.grid} opacity={0.5} />
            <XAxis dataKey="dateLabel" stroke={colors.axis} tick={{ fill: colors.axis, fontSize: 10 }} interval="preserveStartEnd" />
            <YAxis stroke={colors.axis} tick={{ fill: colors.axis, fontSize: 10 }} domain={['auto', 'auto']} tickFormatter={(v) => `${v.toFixed(0)}%`} />
            <Tooltip
              contentStyle={{ backgroundColor: colors.tooltipBg, border: `1px solid ${colors.tooltipBorder}`, borderRadius: '8px' }}
              formatter={(value: number) => [`${Number(value).toFixed(2)}%`, 'Shielded supply %']}
            />
            <Area type="monotone" dataKey="shieldedPct" stroke={colors.shielded} fill={colors.shielded} fillOpacity={0.25} name="Shielded %" />
          </AreaChart>
        )}
      </ResponsiveContainer>
    </ChartCard>
