...
const MANIFEST = [
  ...
  // ---------------------------------------------------------------------
  { method: 'GET', legacyPath: '/api/network/halving', file: 'server/api/routes/network-analytics.js', classification: 'public', domain: 'network', auth: 'none', description: 'Node-observed halving schedule and cadence-based estimates.', v1: { path: '/v1/network/halving', status: 'adapter', shape: 'passthrough', notes: 'A constant-subsidy RPC detector reports a countdown only for an exact integer-zatoshi halving. Other transitions or an exhausted two-million-block search return halvingStatus unavailable with null countdown fields. Missing miner/funding allocations are null. Cadence uses up to 120 consecutive indexed block intervals ending at the observed node height; estimates assume it continues and do not predict future upgrades. Successful complete results may be cached for 300 seconds.' } },
